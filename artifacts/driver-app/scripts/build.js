...
function findWorkspaceRoot(startDir) {
  ...
  while (dir !== path.dirname(dir)) {
    ...
    let hasNpmWorkspace = false;
    const packageJsonPath = path.join(dir, 'package.json');
    if (fs.existsSync(packageJsonPath)) {
      try {
        const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
        hasNpmWorkspace = Array.isArray(packageJson.workspaces);
      } catch {
        // Keep walking if an ancestor package manifest is incomplete.
      }
    }
    if (hasPnpmWorkspace || hasNpmWorkspace) {
      return dir;
    }
    dir = path.dirname(dir);
  }
  throw new Error(
    'Could not find workspace root (no npm workspaces or pnpm-workspace.yaml found)',
  );
}
