...
import { sql, isDatabaseConfigured } from '../lib/sql.js';
import { normalizePdfStoragePayload } from '../lib/pdf-storage.js';
import { assertPublicUrl, fetchWithSsrfGuard } from '../lib/ssrf.js';

// Alias the global fetch Response type so it doesn't conflict with Express's Response.
...
// ── /api/calendar ─────────────────────────────────────────────────────────────
async function handleCalendar(req: Request, res: Response) {
...
// ── /api/deliveries ───────────────────────────────────────────────────────────
async function handleDeliveries(req: Request, res: Response) {
  if (!isDatabaseConfigured()) {
    return res.status(500).json({ success: false, error: 'DATABASE_URL not configured' });
  }
...
// ── /api/notes ────────────────────────────────────────────────────────────────
async function handleNotes(req: Request, res: Response) {
  if (!isDatabaseConfigured()) {
    return res.status(500).json({ success: false, error: 'DATABASE_URL not configured' });
  }
...
// ── /api/plano ────────────────────────────────────────────────────────────────
async function handlePlano(req: Request, res: Response) {
  if (!isDatabaseConfigured()) {
    return res.status(500).json({ success: false, error: 'DATABASE_URL not configured' });
  }
...
// ── /api/proxy-image ──────────────────────────────────────────────────────────
async function handleProxyImage(req: Request, res: Response) {
  const { url } = req.query;
...
async function handleValidateMediaUrl(req: Request, res: Response) {
  if (req.method !== 'GET') {
    return res.status(405).json({ success: false, error: `Method ${req.method} tidak dibenarkan` });
  }
  ...
  const ssrfError = await assertPublicUrl(trimmed);
  if (ssrfError) return res.status(400).json({ success: false, error: ssrfError });
  ...
  try {
    let response = await fetchWithSsrfGuard(trimmed, {
      method: 'HEAD',
      signal: controller.signal,
    ...
    if (response.status === 405 || response.status === 403 || !response.headers.get('content-type')) {
      response = await fetchWithSsrfGuard(trimmed, {
        method: 'GET',
        signal: controller.signal,
...
async function handleUpload(req: Request, res: Response) {
  ...
  const uploadResponse = await fetch(
    `https://api.imgbb.com/1/upload?key=${encodeURIComponent(key)}`,
    {
      ...
      // Buffer is Uint8Array at runtime; cast via unknown to satisfy strict fetch types.
...
// ── /api/pdf-storage ───────────────────────────────────────────────────────
async function handlePdfStorage(req: Request, res: Response) {
  if (!isDatabaseConfigured()) {
    return res.status(500).json({ success: false, error: 'DATABASE_URL not configured' });
  }

  await sql`CREATE TABLE IF NOT EXISTS pdf_storage (
  ...
  if (req.method === 'GET') {
    const { id } = req.query;
    if (id) {
      const result = await sql`SELECT id, filename, mime_type, data, size, created_at, meta FROM pdf_storage WHERE id = ${Number(id)}`;
      if (!result[0]) return res.status(404).json({ success: false, error: 'PDF not found' });
      return res.status(200).json({ success: true, data: result[0] });
    }

    const rows = await sql`SELECT id, filename, mime_type, size, created_at FROM pdf_storage ORDER BY created_at DESC`;
    return res.status(200).json({ success: true, data: rows });
  }

  if (req.method === 'POST') {
    try {
      const normalized = normalizePdfStoragePayload(req.body ?? {});
      const result = await sql`
        INSERT INTO pdf_storage (filename, mime_type, data, size, meta)
        VALUES (${normalized.filename}, ${normalized.mimeType}, ${normalized.data}, ${normalized.size}, ${JSON.stringify(normalized.meta)})
        RETURNING id, filename, mime_type, size, created_at
      `;
      return res.status(200).json({ success: true, data: result[0] });
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Invalid PDF payload';
  ...
  if (req.method === 'DELETE') {
    const { id } = req.query;
    if (!id) return res.status(400).json({ success: false, error: 'id diperlukan' });
    await sql`DELETE FROM pdf_storage WHERE id = ${Number(id)}`;
    return res.status(200).json({ success: true });
  }
...
async function handleShare(req: Request, res: Response) {
  ...
  // Strip both "/api/share/" (original) and "/share/" (after Express removes /api prefix).
...
// ── /api/rooster ──────────────────────────────────────────────────────────────
async function handleRooster(req: Request, res: Response) {
  if (!isDatabaseConfigured()) {
    return res.status(500).json({ success: false, error: 'DATABASE_URL not configured' });
  }
...
// ── /api/route-notes ──────────────────────────────────────────────────────────
async function handleRouteNotes(req: Request, res: Response) {
  if (!isDatabaseConfigured()) {
    return res.status(500).json({ success: false, error: 'DATABASE_URL not configured' });
  }
...
async function osrmFetch(url: string): Promise<FetchResponse> {
  const controller = new AbortController();
...
async function handleRoadDistance(req: Request, res: Response) {
  if (req.method !== 'POST') {
    return res.status(405).json({ success: false, error: `Method ${req.method} not allowed` });
  }
...
async function handleWhatsApp(req: Request, res: Response) {
  if (req.method !== 'POST') {
    return res.status(405).json({ success: false, error: `Method ${req.method} tidak dibenarkan` });
  }
...
// ── /api/routes ───────────────────────────────────────────────────────────────
async function handleRoutes(req: Request, res: Response) {
  if (!isDatabaseConfigured()) {
    return res.status(500).json({ success: false, error: 'DATABASE_URL not configured' });
  }
...
// ── /api/custom-commands ─────────────────────────────────────────────────────
async function handleCustomCommands(req: Request, res: Response) {
  if (!isDatabaseConfigured()) {
    return res.status(500).json({ success: false, error: 'DATABASE_URL not configured' });
  }
...
export async function apiHandler(req: Request, res: Response) {
  ...
  // Extract path segment — works whether req.url is "/api/calendar" or "/calendar"
  const url = req.url ?? '';
  const segment = url.replace(/^\/+api\/+/, '').replace(/^\/+/, '').split('?')[0].split('/')[0];
  ...
  try {
    switch (segment) {
      ...
      case 'pdf-storage':   return await handlePdfStorage(req, res);
